...
const FloatingTourGuideButton: React.FC<FloatingTourGuideButtonProps> = ({ 
  ...
  return (
    <>
      ...
      <div className="fixed bottom-20 right-4 md:right-12 z-50">
        {!isOpen && (
          <div className="relative">
            ...
            <Button
              size="lg"
              className="relative h-14 w-14 md:h-16 md:w-16 rounded-full shadow-2xl bg-gradient-to-br from-monastery-red to-prayer-gold hover:from-monastery-red/90 hover:to-prayer-gold/90 border-2 border-white/20 transition-all duration-300 hover:scale-110 group"
              onClick={() => setIsOpen(true)}
            >
              <svg viewBox="0 0 24 24" className="h-6 w-6 md:h-7 md:w-7 text-white">
                <path 
              ...
              <div className="hidden md:block absolute right-20 top-1/2 -translate-y-1/2 bg-black/80 text-white px-3 py-2 rounded-lg text-sm whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity duration-200 pointer-events-none">
        ...
        {isOpen && (
          <div className="bg-white/95 backdrop-blur-xl border border-white/20 rounded-2xl shadow-2xl 
                          ...
                          sm:w-[350px] sm:h-[550px] 
                          md:w-96 md:h-[600px] 
                          flex flex-col animate-in slide-in-from-right-5 duration-500
                          fixed bottom-4 right-2 
                          sm:bottom-auto sm:right-auto sm:relative">
            
            {/* Header with AI Avatar */}
            <div className="bg-gradient-to-br from-monastery-red to-prayer-gold p-3 md:p-4 rounded-t-2xl border-b border-white/10">
              <div className="flex items-center justify-between">
                <div className="flex items-center space-x-2 md:space-x-3">
                  <Avatar className="h-8 w-8 md:h-10 md:w-10 border-2 border-white/30">
                    <AvatarImage src="/monk-avatar.jpg" />
                    <AvatarFallback className="bg-white/20 text-white font-bold text-xs md:text-sm">🧘</AvatarFallback>
                  </Avatar>
                  <div>
                    <h3 className="font-semibold text-white text-xs md:text-sm">Tenzin - AI Monastery Guide</h3>
                ...
                <div className="flex items-center space-x-1 md:space-x-2">
                  <Button
                    variant="ghost"
                    size="sm"
                    className="h-7 w-7 md:h-8 md:w-8 text-white/80 hover:text-white hover:bg-white/10"
                    onClick={() => setSoundEnabled(!soundEnabled)}
                  >
                    <Volume2 className={`h-3 w-3 md:h-4 md:w-4 ${soundEnabled ? '' : 'line-through'}`} />
                  </Button>
            ...
            {/* Messages Area */}
            <div className="flex-1 overflow-y-auto p-3 md:p-4 space-y-3 md:space-y-4 bg-gradient-to-b from-slate-50 to-white">
              {messages.map((message) => (
                ...
                >
                  <div
                    className={`max-w-[85%] sm:max-w-[280px] rounded-2xl px-3 py-2 md:px-4 md:py-3 ${
                      message.sender === 'user'
                        ? 'bg-gradient-to-br from-monastery-red to-prayer-gold text-white ml-4 md:ml-8'
                        : 'bg-white border border-gray-200 text-gray-800 mr-4 md:mr-8 shadow-sm'
                    } animate-in fade-in slide-in-from-bottom-2 duration-300`}
                  >
                    {message.sender === 'ai' && (
                      <div className="flex items-center space-x-2 mb-1 md:mb-2">
                        <div className="w-5 h-5 md:w-6 md:h-6 rounded-full bg-gradient-to-br from-monastery-red to-prayer-gold flex items-center justify-center">
              ...
              {isTyping && (
                <div className="flex justify-start">
                  <div className="bg-white border border-gray-200 rounded-2xl px-3 py-2 md:px-4 md:py-3 mr-4 md:mr-8 shadow-sm animate-pulse">
            ...
            {/* Enhanced Input Area */}
            <div className="p-3 md:p-4 border-t border-gray-200 bg-white/80 backdrop-blur-sm rounded-b-2xl">
              <div className="flex items-end space-x-2 md:space-x-3">
                <div className="flex-1 relative">
                  <input
                    ...
                    className="w-full px-3 py-2 md:px-4 md:py-3 pr-10 md:pr-12 bg-gray-100 border border-gray-200 rounded-2xl focus:outline-none focus:ring-2 focus:ring-monastery-red focus:border-transparent transition-all duration-200 text-xs md:text-sm"
                    disabled={isTyping}
                  />
                  ...
                  <Button
                    variant="ghost"
                    size="sm"
                    className="absolute right-1 md:right-2 top-1/2 -translate-y-1/2 h-6 w-6 md:h-8 md:w-8 text-gray-400 hover:text-monastery-red"
                ...
                <Button
                  ...
                  className="h-10 w-10 md:h-12 md:w-12 bg-gradient-to-br from-monastery-red to-prayer-gold hover:from-monastery-red/90 hover:to-prayer-gold/90 rounded-2xl transition-all duration-200 hover:scale-105 disabled:opacity-50"
                >
                  <Send className="h-4 w-4 md:h-5 md:w-5 text-white" />
                </Button>
              </div>
              
              {/* Quick Suggestions - Stack on mobile */}
              <div className="flex flex-wrap gap-1 md:gap-2 mt-2 md:mt-3">
                {['🏯 Best monasteries', '🧘 Meditation sessions', '🎭 Festivals', '📍 Nearby locations'].map((suggestion, index) => (
                  <Button
                    ...
                    variant="outline"
                    size="sm"
                    className="text-xs h-6 md:h-7 px-2 md:px-3 rounded-full border-monastery-red/30 text-monastery-red hover:bg-monastery-red/10 flex-shrink-0"
